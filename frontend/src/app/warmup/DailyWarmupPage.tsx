    ...
    return (
      <div className="mx-auto max-w-5xl px-4 py-8">
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900/95 via-slate-950 to-indigo-950/40 px-6 py-10 sm:px-10">
          ...
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex shrink-0 flex-col items-stretch gap-3 sm:items-end">
              <PracticeProgressBadge
                icon={<Flame className="h-5 w-5 shrink-0" style={{ color: streak > 0 ? '#f97316' : '#64748b' }} />}
                caption="Streak"
                primary={streak}
                secondary="days"
              ...
              {userId && credits !== null && (
                <PracticeCostCard
                  ...
                  footer={
                    <Link
                      to="/billing"
                      className="mt-2 block text-center text-xs font-medium text-indigo-300 underline-offset-4 hover:text-indigo-200 hover:underline sm:text-right"
                    >
          ...
          <p className="relative mt-8 max-w-2xl text-center text-xs leading-relaxed text-slate-500 sm:text-left">
            Longer structured tracks live in{' '}
            <Link to="/skills" className="font-medium text-indigo-300 underline-offset-2 hover:text-indigo-200 hover:underline">
              Skill Lab
            </Link>
            . Daily Warmup stays a short, timed ritual — not a substitute for deep practice modules.
          </p>
        </div>
      </div>
    );
      ...
      {phase === 'ready' && (
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs text-slate-500 text-center">
            Credits are charged when you press Start on paid modes (1 / 2 / 3 credits for 30s / 45s / 60s). Use Start first — text-only
