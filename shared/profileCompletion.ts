...
export type AiProfileCriticalField = (typeof AI_PROFILE_CRITICAL_FIELDS)[number];

export type ProfileCompletion = {
  isComplete: boolean;
  completeness: number;
  missingCriticalFields: AiProfileCriticalField[];
};

export type IncompleteProfileResponse = {
  status: 'incomplete_profile';
  reason: 'profile_incomplete';
  message: string;
  missingCriticalFields: AiProfileCriticalField[];
  profileCompletion: ProfileCompletion;
  safeFallbackAllowed: boolean;
};
...
function hasTargetSalary(snapshot: ProfileSnapshot): boolean {
  const goals = snapshot.careerGoals;
  if (!goals) return false;
  if (typeof goals.targetSalary === 'number' && goals.targetSalary > 0) return true;
  return (
    typeof goals.targetSalaryMin === 'number' &&
...
export function evaluateProfileCompletion(snapshot: ProfileSnapshot): ProfileCompletion {
  const missingCriticalFields: AiProfileCriticalField[] = [];

  const targetRole = snapshot.careerGoals?.targetJobTitle ?? snapshot.careerGoals?.strategy?.dreamJob?.targetRole ?? null;
  if (!hasText(targetRole)) missingCriticalFields.push('targetRole');
  if (!hasTargetSalary(snapshot)) missingCriticalFields.push('targetSalary');
...
export function buildIncompleteProfileResponse(
  profileCompletion: ProfileCompletion,
  safeFallbackAllowed = false,
): IncompleteProfileResponse {
  return {
    status: 'incomplete_profile',
    reason: 'profile_incomplete',
    message:
      profileCompletion.missingCriticalFields.length === 0
