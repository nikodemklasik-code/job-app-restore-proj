...
const strategyPatchSchema = z
  .object({
    ...
    roadmap: z.array(z.object({ title: z.string().min(1), done: z.boolean().optional() })).optional(),
    skillCourseLinks: z.array(z.object({ skillName: z.string().min(1), courseId: z.string().optional(), note: z.string().optional() })).optional(),
...
function provenance(source: ProfileFieldProvenance['source'], updatedAt?: Date | string | null, note?: string): ProfileFieldProvenance {
  return { source, updatedAt: updatedAt ? new Date(updatedAt).toISOString() : null, note };
}
...
function normalizeStoredProvenance(raw: unknown): ProfileFieldProvenance | null {
  ...
  if (source !== 'unknown' && source !== 'imported_from_cv' && source !== 'user_confirmed' && source !== 'ai_suggested') return null;
  return {
    source,
...
function experienceProvenance(updatedAt?: Date | string | null): ProfileExperienceProvenance {
  ...
  return { record: base, employerName: base, jobTitle: base, startDate: base, endDate: base, description: base, achievements: base };
}

function educationProvenance(updatedAt?: Date | string | null): ProfileEducationProvenance {
  const base = userConfirmed(updatedAt);
  return { record: base, schoolName: base, degree: base, fieldOfStudy: base, startDate: base, endDate: base };
}

function trainingProvenance(updatedAt?: Date | string | null): ProfileTrainingProvenance {
  const base = userConfirmed(updatedAt);
  return { record: base, title: base, providerName: base, issuedAt: base, expiresAt: base, credentialUrl: base };
}
...
function applyExperienceStoredProvenance(base: ProfileExperienceProvenance, raw: unknown): ProfileExperienceProvenance {
  ...
  return stamp ? { record: stamp, employerName: stamp, jobTitle: stamp, startDate: stamp, endDate: stamp, description: stamp, achievements: stamp } : base;
}

function applyEducationStoredProvenance(base: ProfileEducationProvenance, raw: unknown): ProfileEducationProvenance {
  const stamp = normalizeStoredProvenance(raw);
  return stamp ? { record: stamp, schoolName: stamp, degree: stamp, fieldOfStudy: stamp, startDate: stamp, endDate: stamp } : base;
}

function applyTrainingStoredProvenance(base: ProfileTrainingProvenance, raw: unknown): ProfileTrainingProvenance {
  const stamp = normalizeStoredProvenance(raw);
  return stamp ? { record: stamp, title: stamp, providerName: stamp, issuedAt: stamp, expiresAt: stamp, credentialUrl: stamp } : base;
}
...
async function ensureCareerGoalsRow(userId: string): Promise<void> {
  ...
  await db.insert(careerGoals).values({ id: randomUUID(), userId, autoApplyMinScore: 75 });
}
...
async function ensureSocialConsentsRow(userId: string): Promise<void> {
  ...
  if (existing[0]) return;
  await db.insert(socialConsents).values({ id: randomUUID(), userId });
}

async function ensurePreferenceFlagsRow(userId: string): Promise<void> {
  const existing = await db.select({ userId: userPreferenceFlags.userId }).from(userPreferenceFlags).where(eq(userPreferenceFlags.userId, userId)).limit(1);
  if (existing[0]) return;
...
async function fetchProfileSnapshot(userId: string, email: string): Promise<ProfileSnapshot> {
  ...
  const socialSnapshot: SocialConsentsSnapshot = socialRow[0]
    ? { linkedinConsent: socialRow[0].linkedinConsent, facebookConsent: socialRow[0].facebookConsent, instagramConsent: socialRow[0].instagramConsent }
    : DEFAULT_SOCIAL;
  ...
  if (!profile) {
    return {
      ...
      educations: [],
      trainings: [],
      provenance: { personalInfo: emptyPersonalInfoProvenance(), skills: [], experiences: {}, educations: {}, trainings: {} },
  ...
  const provenanceSnapshot: ProfileSnapshotProvenance = {
    ...
    experiences: Object.fromEntries(experienceRecords.map((e) => [e.id, applyExperienceStoredProvenance(experienceProvenance(e.createdAt ?? profileUpdatedAt), experienceSectionProvenance)])),
    educations: Object.fromEntries(educationRecords.map((e) => [e.id, applyEducationStoredProvenance(educationProvenance(e.createdAt ?? profileUpdatedAt), educationSectionProvenance)])),
    trainings: Object.fromEntries(trainingRecords.map((t) => [t.id, applyTrainingStoredProvenance(trainingProvenance(t.createdAt ?? profileUpdatedAt), trainingSectionProvenance)])),
  };
  ...
  return {
    ...
    experiences: experienceRecords.map((e) => ({ id: e.id, employerName: e.employerName, jobTitle: e.jobTitle, startDate: e.startDate, endDate: e.endDate ?? null, description: e.description ?? '', achievements: normalizeAchievements(e.achievements) })),
    educations: educationRecords.map((e) => ({ id: e.id, schoolName: e.schoolName, degree: e.degree, fieldOfStudy: e.fieldOfStudy ?? '', startDate: e.startDate, endDate: e.endDate ?? null })),
    trainings: trainingRecords.map((t) => ({ id: t.id, title: t.title, providerName: t.providerName, issuedAt: t.issuedAt, expiresAt: t.expiresAt ?? null, credentialUrl: t.credentialUrl ?? '' })),
...
export const profileRouter = router({
  ensureFromClerk: publicProcedure
    .input(z.object({ userId: z.string().min(1), email: z.string().email(), fullName: z.string().optional() }))
    .mutation(async ({ input }) => {
      ...
      if (!row) {
        ...
        await db.insert(users).values({ id: localId, clerkId: input.userId, email: input.email });
      ...
      if (row.email !== input.email) await db.update(users).set({ email: input.email }).where(eq(users.id, row.id));
      ...
      await ensureSocialConsentsRow(row.id);
      await ensurePreferenceFlagsRow(row.id);
      if (!existingProfile[0].fullName?.trim() && displayName) await db.update(profiles).set({ fullName: displayName }).where(eq(profiles.id, existingProfile[0].id));
      return { ok: true, created: false, localUserId: row.id, profileId: existingProfile[0].id };
    }),

  me: protectedProcedure.query(async ({ ctx }) => {
    const user = ctx.user;
    const snapshot = await fetchProfileSnapshot(user.id, user.email);
    return { localUserId: user.id, email: user.email, profile: snapshot };
  }),

  getProfile: protectedProcedure.query(async ({ ctx }) => fetchProfileSnapshot(ctx.user.id, ctx.user.email)),

  updateProfile: protectedProcedure
    .input(z.object({
      personalInfo: z.object({ fullName: z.string(), phone: z.string(), location: z.string(), headline: z.string(), summary: z.string(), linkedinUrl: z.string(), cvUrl: z.string() }),
      skills: z.array(z.string()),
      experiences: z.array(z.object({ employerName: z.string(), jobTitle: z.string(), startDate: z.string(), endDate: z.string().nullable(), description: z.string(), achievements: z.array(z.string()).optional() })),
      educations: z.array(z.object({ schoolName: z.string(), degree: z.string(), fieldOfStudy: z.string(), startDate: z.string(), endDate: z.string().nullable() })),
      trainings: z.array(z.object({ title: z.string(), providerName: z.string(), issuedAt: z.string(), expiresAt: z.string().nullable(), credentialUrl: z.string() })),
      careerGoals: z.object({
        currentJobTitle: z.string().nullable().optional(), currentSalary: z.number().nullable().optional(), targetJobTitle: z.string().nullable().optional(), targetSalary: z.number().nullable().optional(), targetSalaryMin: z.number().nullable().optional(), targetSalaryMax: z.number().nullable().optional(), targetSeniority: z.string().nullable().optional(), workValues: z.array(z.string()).optional(), autoApplyMinScore: z.number().int().min(0).max(100).optional(), strategy: strategyPatchSchema.optional(),
      }).optional(),
      socialConsents: z.object({ linkedinConsent: z.boolean().optional(), facebookConsent: z.boolean().optional(), instagramConsent: z.boolean().optional() }).optional(),
      preferenceFlags: z.object({ caseStudyOptIn: z.boolean().optional(), communityVisibility: z.boolean().optional(), referralParticipation: z.boolean().optional(), sharedSessionsDiscoverable: z.boolean().optional(), aiPersonalizationEnabled: z.boolean().optional() }).optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(profiles).set({ fullName: input.personalInfo.fullName, phone: input.personalInfo.phone, location: input.personalInfo.location, headline: input.personalInfo.headline, summary: input.personalInfo.summary, linkedinUrl: input.personalInfo.linkedinUrl, cvUrl: input.personalInfo.cvUrl }).where(eq(profiles.id, profileId));

      await db.delete(skills).where(eq(skills.profileId, profileId));
      const cleanedSkills = input.skills.map((s) => s.trim()).filter(Boolean);
      if (cleanedSkills.length) await db.insert(skills).values(cleanedSkills.map((name) => ({ id: randomUUID(), profileId, name })));

      await db.delete(experiences).where(eq(experiences.profileId, profileId));
      if (input.experiences.length) await db.insert(experiences).values(input.experiences.map((e) => ({ id: randomUUID(), profileId, employerName: e.employerName, jobTitle: e.jobTitle, startDate: e.startDate, endDate: e.endDate ?? null, description: e.description, achievements: normalizeAchievements(e.achievements) })));

      await db.delete(educations).where(eq(educations.profileId, profileId));
      if (input.educations.length) await db.insert(educations).values(input.educations.map((e) => ({ id: randomUUID(), profileId, schoolName: e.schoolName, degree: e.degree, fieldOfStudy: e.fieldOfStudy, startDate: e.startDate, endDate: e.endDate ?? null })));

      await db.delete(trainings).where(eq(trainings.profileId, profileId));
      if (input.trainings.length) await db.insert(trainings).values(input.trainings.map((t) => ({ id: randomUUID(), profileId, title: t.title, providerName: t.providerName, issuedAt: t.issuedAt, expiresAt: t.expiresAt ?? null, credentialUrl: t.credentialUrl })));

      if (input.careerGoals) {
        const current = await db.select().from(careerGoals).where(eq(careerGoals.userId, localUserId)).limit(1);
        const currentRow = current[0];
        const nextStrategy = { ...(currentRow?.strategyJson && typeof currentRow.strategyJson === 'object' ? currentRow.strategyJson : {}), ...(input.careerGoals.strategy ? normalizeStrategy(input.careerGoals.strategy) : {}) };
        await db.update(careerGoals).set({ currentJobTitle: input.careerGoals.currentJobTitle ?? currentRow?.currentJobTitle ?? null, currentSalary: input.careerGoals.currentSalary ?? currentRow?.currentSalary ?? null, targetJobTitle: input.careerGoals.targetJobTitle ?? currentRow?.targetJobTitle ?? null, targetSalary: input.careerGoals.targetSalary ?? currentRow?.targetSalary ?? null, targetSalaryMin: input.careerGoals.targetSalaryMin ?? currentRow?.targetSalaryMin ?? null, targetSalaryMax: input.careerGoals.targetSalaryMax ?? currentRow?.targetSalaryMax ?? null, targetSeniority: input.careerGoals.targetSeniority ?? currentRow?.targetSeniority ?? null, workValues: input.careerGoals.workValues ? workValuesToDb(input.careerGoals.workValues) : currentRow?.workValues ?? null, autoApplyMinScore: input.careerGoals.autoApplyMinScore ?? currentRow?.autoApplyMinScore ?? 75, strategyJson: nextStrategy }).where(eq(careerGoals.userId, localUserId));
      }
      ...
      if (input.socialConsents) {
        ...
        await db.update(socialConsents).set({ linkedinConsent: input.socialConsents.linkedinConsent ?? rowSocial?.linkedinConsent ?? false, facebookConsent: input.socialConsents.facebookConsent ?? rowSocial?.facebookConsent ?? false, instagramConsent: input.socialConsents.instagramConsent ?? rowSocial?.instagramConsent ?? false }).where(eq(socialConsents.userId, localUserId));
      }
      ...
      if (input.preferenceFlags) {
        ...
        await db.update(userPreferenceFlags).set({ caseStudyOptIn: input.preferenceFlags.caseStudyOptIn ?? rowPref?.caseStudyOptIn ?? false, communityVisibility: input.preferenceFlags.communityVisibility ?? rowPref?.communityVisibility ?? false, referralParticipation: input.preferenceFlags.referralParticipation ?? rowPref?.referralParticipation ?? true, sharedSessionsDiscoverable: input.preferenceFlags.sharedSessionsDiscoverable ?? rowPref?.sharedSessionsDiscoverable ?? false, aiPersonalizationEnabled: input.preferenceFlags.aiPersonalizationEnabled ?? rowPref?.aiPersonalizationEnabled ?? true }).where(eq(userPreferenceFlags.userId, localUserId));
      }

      return { ok: true, profileId };
    }),

  getMatchContext: protectedProcedure.query(async ({ ctx }) => getProfileMatchContextByLocalId(ctx.user.id)),

  isJobBlocked: protectedProcedure
    .input(z.object({ title: z.string().optional(), description: z.string().optional(), company: z.string().optional(), location: z.string().optional() }))
    .query(async ({ ctx, input }) => {
      const context = await getProfileMatchContextByLocalId(ctx.user.id);
      const blocked = isBlockedJob({ title: input.title ?? '', company: input.company ?? null, description: input.description ?? null, seniority: null, salaryMin: null, salaryMax: null, tags: input.location ? [input.location] : [] }, context.blockedAreas);
      return { blocked, reasons: blocked ? context.blockedAreas : [] };
    }),
  ...
  getRoadmap: protectedProcedure.query(async ({ ctx }) => {
    ...
    return generateProfileRoadmap({
      ...
      currentRole: snapshot.careerGoals?.currentJobTitle ?? snapshot.personalInfo.headline ?? null,
