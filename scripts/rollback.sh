...
#     ${REMOTE_BASE}/frontend/dist-backup-<timestamp>
#   This script lists available backups and restores the most recent one.
#
set -euo pipefail

REMOTE_BASE="${REMOTE_BASE:-/root/project}"
DIST_DIR="${REMOTE_BASE}/frontend/dist"
BACKUP_GLOB="${REMOTE_BASE}/frontend/dist-backup-*"
...
TIMESTAMP=$(date +%s)
if [[ -d "$DIST_DIR" ]]; then
  mv "$DIST_DIR" "${REMOTE_BASE}/frontend/dist-pre-rollback-${TIMESTAMP}"
fi
mv "$TARGET" "$DIST_DIR"

echo "✅ Rollback complete — previous dist restored."
echo "   Verify: bash ${REMOTE_BASE}/scripts/smoke-test.sh"
